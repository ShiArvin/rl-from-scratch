...
class BaseAlgorithm(ABC):
    ...
    def __init__(self, training_envs:gym.Env, testing_envs:gym.Env, buffer: ReplayBuffer | TrajectoryRollout, agent: AgentBase, logger: Logger, device, save_pth: str,best_pth:str, args):
        ...
        self.num_training_envs = args.env.num_training_envs #8个并行环境
    ...
    def interact_with_envs(self): #rollout方式在多个并行环境中收集固定步数
        """
        interact with the environments and collect data, then return the collected batch of data
        """
        interact_steps_per_env = self.interact_per_epoch #interact_per_epoch=2000,确定采样的长度
        ...
        with Result("interact") as result:
            for step in range(interact_steps_per_env): #固定循环，8个并行环境，每个环境运行2000步，每个epoch固定收集16000个transitions
    ...
    def run(self):
        ...
        for epoch in tqdm(range(self.total_epoch), desc="Epoch", unit="epoch"):
            ...
            collected_batch, interact_result = self.interact_with_envs() #收集数据
            train_result = self.update(collected_batch, self.start_train()) #更新策略
            if self.train_log_condition(epoch): #可能记录日志
                train_result.add(interact_result)
                self.logger.log_train(epoch, self.interaction_step, self.gradient_step, train_result)
                               
                              
            if self.save_condition(epoch): #可能保存模型
